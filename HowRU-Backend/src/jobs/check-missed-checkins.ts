...
async function escalateToLevel(
  alert: PendingAlert,
  newLevel: 'soft' | 'hard' | 'escalation'
) {
  console.log(
    `[escalate-alerts] Escalating alert ${alert.id} from ${alert.type} to ${newLevel}`
  );
  ...
  // Get checker info
  const checker = (
    await sql`SELECT * FROM users WHERE id = ${alert.checker_id}`
  )[0];
  ...
  const supporters = await sql`
    ...
    ${newLevel === 'soft' ? sql`AND cl.alert_priority = 1` : sql``}
    ORDER BY cl.alert_priority ASC
  `;

  console.log(
    `[escalate-alerts] Notifying ${supporters.length} supporters for ${newLevel} alert`
  );
  ...
  const supporterIds = supporters
    .filter((s: any) => s.supporter_id)
    .map((s: any) => s.supporter_id);
